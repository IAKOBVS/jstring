...
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_search_now_mem_j(const jstr_ty *JSTR_RST const j,
		    regex_t *JSTR_RST const _preg,
		    regmatch_t *JSTR_RST const _pmatch,
		    const int _eflags) JSTR_NOEXCEPT
{
	return jreg_search_now_mem(j->data, j->size, _preg, _pmatch, _eflags);
}
...
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_search_mem_j(const jstr_ty *JSTR_RST const j,
		const char *JSTR_RST const _ptn,
		regex_t *JSTR_RST const _preg,
		regmatch_t *JSTR_RST const _pmatch,
		const int _cflags,
		const int _eflags) JSTR_NOEXCEPT
{
	return jreg_search_mem(j->data, _ptn, j->size, _preg, _pmatch, _cflags, _eflags);
}
...
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_match_now_mem_j(const jstr_ty *JSTR_RST const j,
		   const char *JSTR_RST const _ptn,
		   regex_t *JSTR_RST const _preg,
		   const int _cflags,
		   const int _eflags) JSTR_NOEXCEPT
{
	return jreg_match_now_mem(j->data, _ptn, j->size, _preg, _cflags, _eflags);
}
...
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_rm_j(jstr_ty *JSTR_RST const j,
	const regex_t *JSTR_RST const _preg,
	const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rm(j->data, &j->size, _preg, _eflags);
}
...
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_rm_now_j(jstr_ty *JSTR_RST const j,
	    const char *JSTR_RST const _ptn,
	    regex_t *JSTR_RST const _preg,
	    const int _cflags,
	    const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rm_now(j->data, &j->size, _ptn, _preg, _cflags, _eflags);
}
...
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_rmall_j(jstr_ty *JSTR_RST const j,
	   const regex_t *JSTR_RST const _preg,
	   const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rmall(j->data, &j->size, _preg, _eflags);
}
...
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_rmn_j(jstr_ty *JSTR_RST const j,
	 const size_t _n,
	 const regex_t *JSTR_RST const _preg,
	 const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rmn(j->data, &j->size, _n, _preg, _eflags);
}
...
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_rmn_now_j(jstr_ty *JSTR_RST const j,
	     const char *JSTR_RST const _ptn,
	     const size_t _n,
	     regex_t *JSTR_RST const _preg,
	     const int _cflags,
	     const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rmn_now(j->data, _ptn, &j->size, _n, _preg, _cflags, _eflags);
}
...
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_rmall_now_j(jstr_ty *JSTR_RST const j,
	       const char *JSTR_RST const _ptn,
	       regex_t *JSTR_RST const _preg,
	       const int _cflags,
	       const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rmall_now(j->data, &j->size, _ptn, _preg, _cflags, _eflags);
}
...
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_rplcall_mem_j(jstr_ty *JSTR_RST const j,
		 const char *JSTR_RST const _rplc,
		 const size_t _rplclen,
		 const regex_t *JSTR_RST const _preg,
		 const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplcall_mem(&j->data, &j->size, &j->cap, _rplc, _rplclen, _preg, _eflags);
}
...
jreg_rplcall(char **JSTR_RST const _s,
		 ...
		 const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplcall_mem(_s, _sz, _cap, _rplc, strlen(_rplc), _preg, _eflags);
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_rplcall_j(jstr_ty *JSTR_RST const j,
		 const char *JSTR_RST const _rplc,
		 const regex_t *JSTR_RST const _preg,
		 const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplcall(&j->data, &j->size, &j->cap, _rplc, _preg, _eflags);
}
...
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_rplcn_mem_j(jstr_ty *JSTR_RST const j,
	       const char *JSTR_RST const _rplc,
	       const size_t _n,
	       const size_t _rplclen,
	       const regex_t *JSTR_RST const _preg,
	       const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplcn_mem(&j->data, &j->size, &j->cap, _rplc, _n, _rplclen, _preg, _eflags);
}
...
jreg_rplcn(char **JSTR_RST const _s,
	       ...
	       const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplcn_mem(_s, _sz, _cap, _rplc, _n, strlen(_rplc), _preg, _eflags);
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_rplcn_j(jstr_ty *JSTR_RST const j,
	       const char *JSTR_RST const _rplc,
	       const size_t _n,
	       const regex_t *JSTR_RST const _preg,
	       const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplcn(&j->data, &j->size, &j->cap, _rplc, _n, _preg, _eflags);
}
...
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_rplcall_now_mem_j(jstr_ty *JSTR_RST const j,
		     const char *JSTR_RST const _ptn,
		     const char *JSTR_RST const _rplc,
		     const size_t _rplclen,
		     regex_t *JSTR_RST const _preg,
		     ...
		     const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplcall_now_mem(&j->data, &j->size, &j->cap, _ptn, _rplc, _rplclen, _preg, _cflags, _eflags);
}
...
jreg_rplcall_now(char **JSTR_RST const _s,
		     ...
		     const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplcall_now_mem(_s, _sz, _cap, _ptn, _rplc, strlen(_rplc), _preg, _cflags, _eflags);
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_rplcall_now_j(jstr_ty *JSTR_RST const j,
		     const char *JSTR_RST const _ptn,
		     const char *JSTR_RST const _rplc,
		     regex_t *JSTR_RST const _preg,
		     const int _cflags,
		     const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplcall_now(&j->data, &j->size, &j->cap, _ptn, _rplc, _preg, _cflags, _eflags);
}
...
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_rplcn_now_mem_j(jstr_ty *JSTR_RST const j,
		   const char *JSTR_RST const _ptn,
		   const char *JSTR_RST const _rplc,
		   const size_t _n,
		   const size_t _rplclen,
		   ...
		   const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplcn_now_mem(&j->data, &j->size, &j->cap, _ptn, _rplc, _n, _rplclen, _preg, _cflags, _eflags);
}
...
jreg_rplcn_now(char **JSTR_RST const _s,
		   ...
		   const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplcn_now_mem(_s, _sz, _cap, _ptn, _rplc, _n, strlen(_rplc), _preg, _cflags, _eflags);
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_rplcn_now_j(jstr_ty *JSTR_RST const j,
		   const char *JSTR_RST const _ptn,
		   const char *JSTR_RST const _rplc,
		   const size_t _n,
		   regex_t *JSTR_RST const _preg,
		   ...
		   const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplcn_now(&j->data, &j->size, &j->cap, _ptn, _rplc, _n, _preg, _cflags, _eflags);
}
...
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_rplc_mem_j(jstr_ty *JSTR_RST const j,
	      const char *JSTR_RST const _rplc,
	      const size_t _rplclen,
	      const regex_t *JSTR_RST const _preg,
	      const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplc_mem(&j->data, &j->size, &j->cap, _rplc, _rplclen, _preg, _eflags);
}
...
jreg_rplc(char **JSTR_RST const _s,
	      ...
	      const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplc_mem(_s, _sz, _cap, _rplc, strlen(_rplc), _preg, _eflags);
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_rplc_j(jstr_ty *JSTR_RST const j,
	      const char *JSTR_RST const _rplc,
	      const regex_t *JSTR_RST const _preg,
	      const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplc(&j->data, &j->size, &j->cap, _rplc, _preg, _eflags);
}
...
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_rplc_now_mem_j(jstr_ty *JSTR_RST const j,
		  const char *JSTR_RST const _ptn,
		  const char *JSTR_RST const _rplc,
		  const size_t _rplclen,
		  regex_t *JSTR_RST const _preg,
		  ...
		  const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplc_now_mem(&j->data, &j->size, &j->cap, _ptn, _rplc, _rplclen, _preg, _cflags, _eflags);
}
...
jreg_rplc_now(char **JSTR_RST const _s,
		  ...
		  const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplc_now_mem(_s, _sz, _cap, _ptn, _rplc, strlen(_rplc), _preg, _cflags, _eflags);
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static jreg_errcode_ty
jreg_rplc_now_j(jstr_ty *JSTR_RST const j,
		  const char *JSTR_RST const _ptn,
		  const char *JSTR_RST const _rplc,
		  regex_t *JSTR_RST const _preg,
		  const int _cflags,
		  const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplc_now(&j->data, &j->size, &j->cap, _ptn, _rplc, _preg, _cflags, _eflags);
}
