			   ...
			   const size_t _rplclen) JSTR_NOEXCEPT
{
	if (*_cap < *_sz + _rplclen)
		PRIV_JSTR_REALLOC(*_s, *_cap, *_sz + _rplclen, return NULL);
			  ...
			  const size_t _rplclen) JSTR_NOEXCEPT
{
	if (*_cap > *_sz + _rplclen)
		return jstr_slip_mem_p_f(*_s, _at, _rplc, *_sz, _rplclen);
	PRIV_JSTR_GROW(*_cap, *_sz + _rplclen);
	char *const _tmp = (char *)malloc(*_cap);
	PRIV_JSTR_MALLOC_ERR(_tmp, return NULL);
	memcpy(_tmp, *_s, _at);
			    ...
			    const size_t _findlen) JSTR_NOEXCEPT
{
	if (*_cap > *_sz + _rplclen - _findlen)
		return jstr_rplcat_mem_f(*_s, _sz, _at, _rplc, _rplclen, _findlen);
	PRIV_JSTR_GROW(*_cap, *_sz + _rplclen - _findlen);
	char *const _tmp = (char *)malloc(*_cap);
	PRIV_JSTR_MALLOC_ERR(_tmp, return NULL);
	memcpy(_tmp, *_s, _at);
	memcpy(_tmp + _at, _rplc, _rplclen);
...
#if JSTR_HAVE_REALLOC_MREMAP
		if (JSTR_IS_MMAP(*_cap)) {
			PRIV_JSTR_REALLOC(*_s, *_cap, *_sz + _rplclen - _findlen, return NULL);
			return jstr_rplcat_mem_f(*_s, _sz, _at, _rplc, _rplclen, _findlen);
		}
#else
		PRIV_JSTR_GROW(*_cap, *_sz + _rplclen - _findlen);
...
JSTR_MAYBE_UNUSED
static void
jstr_slipaftc_mem_j(jstr_ty *JSTR_RST const j,
		  const int _c,
		  const char *JSTR_RST const _src,
		  const size_t _srclen) JSTR_NOEXCEPT
{
	jstr_slipaftc_mem(&j->data, &j->size, &j->cap, _c, _src, _srclen);
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static void
jstr_slipaftc(char **JSTR_RST const _s,
		  size_t *JSTR_RST const _sz,
		  size_t *JSTR_RST const _cap,
		  const int _c,
		  const char *JSTR_RST const _src) JSTR_NOEXCEPT
{
	jstr_slipaftc_mem(_s, _sz, _cap, _c, _src, strlen(_src));
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static void
jstr_slipaftc_j(jstr_ty *JSTR_RST const j,
		  const int _c,
		  const char *JSTR_RST const _src) JSTR_NOEXCEPT
{
	jstr_slipaftc(&j->data, &j->size, &j->cap, _c, _src);
}
...
JSTR_MAYBE_UNUSED
static void
jstr_slipaftallc_mem_j(jstr_ty *JSTR_RST const j,
		     const int _c,
		     const char *JSTR_RST const _src,
		     const size_t _srclen) JSTR_NOEXCEPT
{
	jstr_slipaftallc_mem(&j->data, &j->size, &j->cap, _c, _src, _srclen);
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static void
jstr_slipaftallc(char **JSTR_RST const _s,
		     size_t *JSTR_RST const _sz,
		     size_t *JSTR_RST const _cap,
		     const int _c,
		     const char *JSTR_RST const _src) JSTR_NOEXCEPT
{
	jstr_slipaftallc_mem(_s, _sz, _cap, _c, _src, strlen(_src));
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static void
jstr_slipaftallc_j(jstr_ty *JSTR_RST const j,
		     const int _c,
		     const char *JSTR_RST const _src) JSTR_NOEXCEPT
{
	jstr_slipaftallc(&j->data, &j->size, &j->cap, _c, _src);
}
		 ...
		 const size_t _srclen) JSTR_NOEXCEPT
{
	...
	case 0: return;
	case 1:
		jstr_slipaftc_mem(_s, _sz, _cap, *_find, _src, _srclen);
		return;
	default: {
		const char *const _p = (char *)PRIV_JSTR_MEMMEM(*_s, *_sz, _find, _findlen);
		if (_p != NULL)
...
JSTR_MAYBE_UNUSED
static void
jstr_slipaft_mem_j(jstr_ty *JSTR_RST const j,
		 const char *JSTR_RST const _find,
		 const char *JSTR_RST const _src,
		 const size_t _findlen,
		 const size_t _srclen) JSTR_NOEXCEPT
{
	jstr_slipaft_mem(&j->data, &j->size, &j->cap, _find, _src, _findlen, _srclen);
}
...
jstr_slipaft(char **JSTR_RST const _s,
		 ...
		 const char *JSTR_RST const _src) JSTR_NOEXCEPT
{
	return jstr_slipaft_mem(_s, _sz, _cap, _find, _src, strlen(_find), strlen(_src));
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static void
jstr_slipaft_j(jstr_ty *JSTR_RST const j,
		 const char *JSTR_RST const _find,
		 const char *JSTR_RST const _src) JSTR_NOEXCEPT
{
	jstr_slipaft(&j->data, &j->size, &j->cap, _find, _src);
}
			...
			const size_t _srclen) JSTR_NOEXCEPT
{
	switch (_findlen) {
	case 0: return _s + _sz;
	case 1: return jstr_slipaftallc_mem_p_f(_s, *_find, _src, _sz, _srclen);
	default: {
		size_t off = 0;
		const char *_p;
		while ((_p = (char *)PRIV_JSTR_MEMMEM(_s + off, _sz - off, _find, _findlen))) {
			off = _p - _s;
			_sz = jstr_slip_mem_p_f(_s, off, _src, _sz, _srclen) - _s;
		    ...
		    const size_t _srclen) JSTR_NOEXCEPT
{
	...
	case 0:
		return;
	case 1:
		jstr_slipaftallc_mem(_s, _sz, _cap, *_find, _src, _srclen);
		return;
	default: {
		size_t off = 0;
		const char *_p;
...
JSTR_MAYBE_UNUSED
static void
jstr_slipaftall_mem_j(jstr_ty *JSTR_RST const j,
		    const char *JSTR_RST const _find,
		    const char *JSTR_RST const _src,
		    const size_t _findlen,
		    const size_t _srclen) JSTR_NOEXCEPT
{
	jstr_slipaftall_mem(&j->data, &j->size, &j->cap, _find, _src, _findlen, _srclen);
}
...
jstr_slipaftall(char **JSTR_RST const _s,
		    ...
		    const char *JSTR_RST const _src) JSTR_NOEXCEPT
{
	return jstr_slipaftall_mem(_s, _sz, _cap, _find, _src, strlen(_find), strlen(_src));
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static void
jstr_slipaftall_j(jstr_ty *JSTR_RST const j,
		    const char *JSTR_RST const _find,
		    const char *JSTR_RST const _src) JSTR_NOEXCEPT
{
	jstr_slipaftall(&j->data, &j->size, &j->cap, _find, _src);
}
...
JSTR_RETURNS_NONNULL
static char *
priv_jstr_rmallc_mem_p(const priv_jstr_flag_use_n_ty _flag,
...
jstr_rmallc_mem_p(char *JSTR_RST const _s,
		  ...
		  const size_t _sz) JSTR_NOEXCEPT
{
	return priv_jstr_rmallc_mem_p(PRIV_JSTR_FLAG_USE_NOT_N, _s, _c, 0, _sz);
}
...
jstr_rmnc_mem_p(char *JSTR_RST const _s,
		...
		const size_t _sz) JSTR_NOEXCEPT
{
	return priv_jstr_rmallc_mem_p(PRIV_JSTR_FLAG_USE_N, _s, _c, _n, _sz);
}
	      ...
	      const size_t _findlen) JSTR_NOEXCEPT
{
	if (jstr_unlikely(_findlen == 0))
		return _s + _sz;
	char *const _p = (char *)PRIV_JSTR_MEMMEM(_s, _sz, _find, _findlen);
...
JSTR_MAYBE_UNUSED
static void
jstr_rplcc_mem_j(jstr_ty *JSTR_RST const j,
	       const int _find,
	       const int _rplc) JSTR_NOEXCEPT
{
	jstr_rplcc_mem(j->data, _find, _rplc, j->size);
}
...
JSTR_MAYBE_UNUSED
static void
jstr_rplcallc_mem_j(jstr_ty *JSTR_RST const j,
		  const int _find,
		  const int _rplc) JSTR_NOEXCEPT
{
	jstr_rplcallc_mem(j->data, _find, _rplc, j->size);
}
...
JSTR_MAYBE_UNUSED
static void
jstr_rplcnc_mem_j(jstr_ty *JSTR_RST const j,
		const int _find,
		const int _rplc,
		size_t _n) JSTR_NOEXCEPT
{
	jstr_rplcnc_mem(j->data, _find, _rplc, _n, j->size);
}
		  ...
		  const size_t _rplclen) JSTR_NOEXCEPT
{
	switch (_rplclen) {
	case 0:
		return jstr_rm_mem_p(_s, _find, _sz, _findlen);
	case 1:
		if (jstr_unlikely(_findlen == 1)) {
			jstr_rplcc_mem(_s, *_find, *_rplc, _sz);
			return _s + _sz;
		}
		/* FALLTHROUGH */
	default: {
		if (jstr_unlikely(_findlen == 0))
			return _s + _sz;
		char *_p = (char *)PRIV_JSTR_MEMMEM(_s, _sz, _find, _findlen);
		if (jstr_unlikely(_p == NULL))
			return _s + _sz;
		memmove(_p + _rplclen,
			_p + _findlen,
			(_s + _sz + 1) - _p + _findlen);
		memcpy(_p, _rplc, _rplclen);
		return _s + _sz - _rplclen - _findlen;
	}
	}
}
...
JSTR_MAYBE_UNUSED
static void
jstr_rplc_mem_j(jstr_ty *JSTR_RST const j,
	      const char *JSTR_RST const _find,
	      const char *JSTR_RST const _rplc,
	      const size_t _findlen,
	      const size_t _rplclen) JSTR_NOEXCEPT
{
	jstr_rplc_mem(&j->data, &j->size, &j->cap, _find, _rplc, _findlen, _rplclen);
}
...
jstr_rplc(char **JSTR_RST const _s,
	      ...
	      size_t *JSTR_RST const _cap,
	      const char *JSTR_RST const _find,
	      const char *JSTR_RST const _rplc) JSTR_NOEXCEPT
{
	return jstr_rplc_mem(_s, _sz, _cap, _find, _rplc, strlen(_find), strlen(_rplc));
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static void
jstr_rplc_j(jstr_ty *JSTR_RST const j,
	      const char *JSTR_RST const _find,
	      const char *JSTR_RST const _rplc) JSTR_NOEXCEPT
{
	jstr_rplc(&j->data, &j->size, &j->cap, _find, _rplc);
}
		  ...
		  const size_t _rplclen) JSTR_NOEXCEPT
{
	if (jstr_unlikely(_findlen == 0))
		return;
	char *_p = (char *)jstr_memrmem(*_s, *_sz, _find, _findlen);
	if (jstr_unlikely(_p == NULL))
		return;
...
JSTR_MAYBE_UNUSED
static void
jstr_rplclast_mem_j(jstr_ty *JSTR_RST const j,
		  const char *JSTR_RST const _find,
		  const char *JSTR_RST const _rplc,
		  const size_t _findlen,
		  const size_t _rplclen) JSTR_NOEXCEPT
{
	jstr_rplclast_mem(&j->data, &j->size, &j->cap, _find, _rplc, _findlen, _rplclen);
}
...
jstr_rplclast(char **JSTR_RST const _s,
		  ...
		  const char *JSTR_RST const _rplc) JSTR_NOEXCEPT
{
	return jstr_rplclast_mem(_s, _sz, _cap, _find, _rplc, strlen(_find), strlen(_rplc));
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static void
jstr_rplclast_j(jstr_ty *JSTR_RST const j,
		  const char *JSTR_RST const _find,
		  const char *JSTR_RST const _rplc) JSTR_NOEXCEPT
{
	jstr_rplclast(&j->data, &j->size, &j->cap, _find, _rplc);
}
...
{
	if (jstr_unlikely(_findlen == 1))
		return priv_jstr_rmallc_mem_p(_flag, _s, *_find, _n, _sz);
	if (jstr_unlikely(_findlen == 0))
		return _s + _sz;
	...
	const unsigned char *_p = _dst;
	const unsigned char *const _end = _dst + _sz;
	while ((_p = (unsigned char *)PRIV_JSTR_MEMMEM((char *)_p, _end - _p, _find, _findlen))) {
...
jstr_rmn_mem_p(char *JSTR_RST const _s,
	       ...
	       const size_t _findlen) JSTR_NOEXCEPT
{
	return priv_jstr_rmall_mem_p(PRIV_JSTR_FLAG_USE_N, _s, _find, _n, _sz, _findlen);
}
...
jstr_rmall_mem_p(char *JSTR_RST const _s,
		 ...
		 const size_t _findlen) JSTR_NOEXCEPT
{
	return priv_jstr_rmall_mem_p(PRIV_JSTR_FLAG_USE_NOT_N, _s, _find, 0, _sz, _findlen);
}
		      ...
		      const size_t _rplclen) JSTR_NOEXCEPT
{
	if (jstr_unlikely(_rplclen == 0)) {
		*_sz = priv_jstr_rmall_mem_p(_flag, *_s, _find, _n, *_sz, _findlen) - *_s;
		return;
	}
	...
	const int is_mmap = JSTR_IS_MMAP(*_cap);
#endif /* JSTR_HAVE_REALLOC_MREMAP */
	while ((_p = (uc *)PRIV_JSTR_MEMMEM((char *)_p, (*(uc **)_s + *_sz) - _p, _find, _findlen))) {
...
jstr_rplcn_mem(char **JSTR_RST const _s,
	       ...
	       const size_t _rplclen) JSTR_NOEXCEPT
{
	priv_jstr_rplcall_mem(PRIV_JSTR_FLAG_USE_N, _s, _sz, _cap, _find, _rplc, _n, _findlen, _rplclen);
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static void
jstr_rplcn_mem_j(jstr_ty *JSTR_RST const j,
	       const char *JSTR_RST const _find,
	       const char *JSTR_RST const _rplc,
	       size_t _n,
	       const size_t _findlen,
	       const size_t _rplclen) JSTR_NOEXCEPT
{
	jstr_rplcn_mem(&j->data, &j->size, &j->cap, _find, _rplc, _n, _findlen, _rplclen);
}
...
jstr_rplcn(char **JSTR_RST const _s,
	       ...
	       size_t _n) JSTR_NOEXCEPT
{
	jstr_rplcn_mem(_s, _sz, _cap, _find, _rplc, _n, strlen(_find), strlen(_rplc));
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static void
jstr_rplcn_j(jstr_ty *JSTR_RST const j,
	       const char *JSTR_RST const _find,
	       const char *JSTR_RST const _rplc,
	       size_t _n) JSTR_NOEXCEPT
{
	jstr_rplcn(&j->data, &j->size, &j->cap, _find, _rplc, _n);
}
...
jstr_rplcall_mem(char **JSTR_RST const _s,
		 ...
		 const size_t _rplclen) JSTR_NOEXCEPT
{
	priv_jstr_rplcall_mem(PRIV_JSTR_FLAG_USE_NOT_N, _s, _sz, _cap, _find, _rplc, 0, _findlen, _rplclen);
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static void
jstr_rplcall_mem_j(jstr_ty *JSTR_RST const j,
		 const char *JSTR_RST const _find,
		 const char *JSTR_RST const _rplc,
		 const size_t _findlen,
		 const size_t _rplclen) JSTR_NOEXCEPT
{
	jstr_rplcall_mem(&j->data, &j->size, &j->cap, _find, _rplc, _findlen, _rplclen);
}
...
jstr_rplcall(char **JSTR_RST const _s,
		 ...
		 const char *JSTR_RST const _rplc) JSTR_NOEXCEPT
{
	jstr_rplcall_mem(_s, _sz, _cap, _find, _rplc, strlen(_find), strlen(_rplc));
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static void
jstr_rplcall_j(jstr_ty *JSTR_RST const j,
		 const char *JSTR_RST const _find,
		 const char *JSTR_RST const _rplc) JSTR_NOEXCEPT
{
	jstr_rplcall(&j->data, &j->size, &j->cap, _find, _rplc);
}
...
JSTR_MAYBE_UNUSED
static void
jstr_rev_mem_j(jstr_ty *JSTR_RST const j) JSTR_NOEXCEPT
{
	jstr_rev_mem(j->data, j->size);
}
...
{
	return jstr_rev_mem(_s, strlen(_s));
}
...
JSTR_MAYBE_UNUSED
static void
jstr_insert_f(char *JSTR_RST const _s,
		  const size_t _at,
		  const char *JSTR_RST const _src) JSTR_NOEXCEPT
{
	jstr_insert_mem_f(_s, _at, _src, strlen(_src));
}
...
JSTR_MAYBE_UNUSED
static void
jstr_insert_mem_j(jstr_ty *JSTR_RST const j,
		const size_t _at,
		const char *JSTR_RST const _src,
		const size_t _srclen) JSTR_NOEXCEPT
{
	jstr_insert_mem(&j->data, &j->size, &j->cap, _at, _src, _srclen);
}
...
jstr_insert(char **JSTR_RST const _s,
		...
		const char *JSTR_RST const _src) JSTR_NOEXCEPT
{
	return jstr_insert_mem(_s, _sz, _cap, _at, _src, strlen(_src));
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static void
jstr_insert_j(jstr_ty *JSTR_RST const j,
		const size_t _at,
		const char *JSTR_RST const _src) JSTR_NOEXCEPT
{
	jstr_insert(&j->data, &j->size, &j->cap, _at, _src);
}
...
JSTR_MAYBE_UNUSED
static void
jstr_insertaftc_mem_f_j(jstr_ty *JSTR_RST const j,
		      const int _c,
		      const char *JSTR_RST const _src,
		      const size_t _srclen) JSTR_NOEXCEPT
{
	jstr_insertaftc_mem_f(j->data, _c, _src, j->size, _srclen);
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static void
jstr_insertaftc_f(char *JSTR_RST const _s,
		      const int _c,
		      const char *JSTR_RST const _src,
		      const size_t _sz) JSTR_NOEXCEPT
{
	jstr_insertaftc_mem_f(_s, _c, _src, _sz, strlen(_src));
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static void
jstr_insertaftc_f_j(jstr_ty *JSTR_RST const j,
		      const int _c,
		      const char *JSTR_RST const _src) JSTR_NOEXCEPT
{
	jstr_insertaftc_f(j->data, _c, _src, j->size);
}
...
JSTR_MAYBE_UNUSED
static void
jstr_insertaftc_mem_j(jstr_ty *JSTR_RST const j,
		    const int _c,
		    const char *JSTR_RST const _src,
		    const size_t _srclen) JSTR_NOEXCEPT
{
	jstr_insertaftc_mem(&j->data, &j->size, &j->cap, _c, _src, _srclen);
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static void
jstr_insertaftc(char **JSTR_RST const _s,
		    size_t *JSTR_RST const _sz,
		    size_t *JSTR_RST const _cap,
		    const int _c,
		    const char *JSTR_RST const _src) JSTR_NOEXCEPT
{
	jstr_insertaftc_mem(_s, _sz, _cap, _c, _src, strlen(_src));
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static void
jstr_insertaftc_j(jstr_ty *JSTR_RST const j,
		    const int _c,
		    const char *JSTR_RST const _src) JSTR_NOEXCEPT
{
	jstr_insertaftc(&j->data, &j->size, &j->cap, _c, _src);
}
...
JSTR_MAYBE_UNUSED
static void
jstr_insertaft_mem_f_j(jstr_ty *JSTR_RST const j,
		     const char *JSTR_RST const _find,
		     const char *JSTR_RST const _src,
		     const size_t _findlen,
		     const size_t _srclen) JSTR_NOEXCEPT
{
	jstr_insertaft_mem_f(j->data, _find, _src, j->size, _findlen, _srclen);
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static void
jstr_insertaft_f(char *JSTR_RST const _s,
		     const char *JSTR_RST const _find,
		     const char *JSTR_RST const _src,
		     const size_t _sz) JSTR_NOEXCEPT
{
	return jstr_insertaft_mem_f(_s, _find, _src, _sz, strlen(_find), strlen(_src));
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static void
jstr_insertaft_f_j(jstr_ty *JSTR_RST const j,
		     const char *JSTR_RST const _find,
		     const char *JSTR_RST const _src) JSTR_NOEXCEPT
{
	jstr_insertaft_f(j->data, _find, _src, j->size);
}
...
JSTR_MAYBE_UNUSED
static void
jstr_insertaft_mem_j(jstr_ty *JSTR_RST const j,
		   const char *JSTR_RST const _find,
		   const char *JSTR_RST const _src,
		   const size_t _findlen,
		   const size_t _srclen) JSTR_NOEXCEPT
{
	jstr_insertaft_mem(&j->data, &j->size, &j->cap, _find, _src, _findlen, _srclen);
}
...
jstr_insertaft(char **JSTR_RST const _s,
		   ...
		   const char *JSTR_RST const _src) JSTR_NOEXCEPT
{
	return jstr_insertaft_mem(_s, _sz, _cap, _find, _src, strlen(_find), strlen(_src));
}

JSTR_INLINE
JSTR_MAYBE_UNUSED
static void
jstr_insertaft_j(jstr_ty *JSTR_RST const j,
		   const char *JSTR_RST const _find,
		   const char *JSTR_RST const _src) JSTR_NOEXCEPT
{
	jstr_insertaft(&j->data, &j->size, &j->cap, _find, _src);
}
