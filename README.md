...
- string formatting: asprintf\(\) which returns the size of allocation and vsprintfstrlenmax\(\)
which returns the maximum size of allocation needed to store the characters written by vsprintf
...
- compile: run setup checks, generate headers, and build the shared library into `build/`.
- compile-native: build with `-march=native`.
- test: run all tests (each test is compiled in multiple variants).
- test-check-fail: run all tests, printing only failures.
- bench/bench: compile and run benchmarks in `bench/`; results land in `bench/result-*` (sort with `bench/see`).
- install: copy headers to /usr/local/include/jstr and the library to /usr/local/lib \(requires sudo\).
- install-to: install to a specified directory.
- uninstall: remove installed files \(requires sudo\).
- clean: remove files generated by compile.
