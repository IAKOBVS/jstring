...
Use jstr\_err\(\) to print the error message or jstr\_err\_exit() to also exit. For more comprehensive error messages, use the \*\_debug\(\) functions or do
```
define JSTR_DEBUG 1
```
This will also print the filename, line number, and function in which the error occured.
