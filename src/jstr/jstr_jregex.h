...
JSTR_INLINE
static jreg_errcode_ty
jreg_exec_len(const regex_t *JSTR_RST _preg,
	      const char *JSTR_RST const _s,
	      const size_t _sz,
	      const size_t _nmatch,
	      regmatch_t *JSTR_RST const _pmatch,
...
#ifdef JREG_EF_STARTEND
#	define PJREG_EXEC(_preg, _s, _sz, _nmatch, _pmatch, _eflags) \
		jreg_exec_len(_preg, _s, _sz, _nmatch, _pmatch, _eflags)
#else
#	define PJREG_EXEC(_preg, _s, _sz, _nmatch, _pmatch, _eflags) \
...
JSTR_INLINE
static jreg_errcode_ty
jreg_search_now_len(const char *JSTR_RST const _s,
		    const size_t _sz,
		    regex_t *JSTR_RST const _preg,
		    regmatch_t *JSTR_RST const _pmatch,
		    const int _eflags) JSTR_NOEXCEPT
{
	return jreg_exec_len(_preg, _s, _sz, 1, _pmatch, _eflags | JREG_EF_NOSUB);
}
...
JSTR_INLINE
static jreg_errcode_ty
jreg_search_len(const char *JSTR_RST const _s,
		const char *JSTR_RST const _ptn,
		const size_t _sz,
		regex_t *JSTR_RST const _preg,
		regmatch_t *JSTR_RST const _pmatch,
		...
		const int _eflags) JSTR_NOEXCEPT
{
	...
	if (jstr_unlikely(_ret != JREG_RET_NOERROR))
		return _ret;
	return jreg_search_now_len(_s, _sz, _preg, _pmatch, _eflags);
}
/*
   Checks if S matches PTN.
   Returns return value of regexec or regcomp if it fails.
...
JSTR_INLINE
static jreg_errcode_ty
jreg_match_len(const char *JSTR_RST const _s,
	       const size_t _sz,
	       regex_t *JSTR_RST const _preg,
	       const int _eflags) JSTR_NOEXCEPT
{
	regmatch_t _rm;
	return jreg_exec_len(_preg, _s, _sz, 0, &_rm, _eflags | JREG_EF_NOSUB | JREG_EF_STARTEND);
}
...
JSTR_NOTHROW
static jreg_errcode_ty
jreg_match_now_len(const char *JSTR_RST const _s,
		   const char *JSTR_RST const _ptn,
		   const size_t _sz,
		   regex_t *JSTR_RST const _preg,
		   const int _cflags,
		   const int _eflags) JSTR_NOEXCEPT
{
	...
	if (jstr_unlikely(_ret != JREG_RET_NOERROR))
		return _ret;
	return jreg_match_len(_s, _sz, _preg, _eflags);
}
...
jreg_rmall(char *JSTR_RST const _s,
	   ...
	   const int _eflags) JSTR_NOEXCEPT
{
	return pjreg_base_rmall(PJSTR_FLAG_USE_NOT_N, _s, _sz, 0, _preg, _eflags);
}
...
jreg_rmn(char *JSTR_RST const _s,
	 ...
	 const int _eflags) JSTR_NOEXCEPT
{
	return pjreg_base_rmall(PJSTR_FLAG_USE_N, _s, _sz, _n, _preg, _eflags);
}
...
JSTR_FUNC
static jreg_errcode_ty
pjreg_base_rplcall_len(const pjstr_flag_use_n_ty _flag,
		       char **JSTR_RST const _s,
		       size_t *JSTR_RST const _sz,
		       ...
		       const int _eflags) JSTR_NOEXCEPT
{
	...
	while ((_flag & PJSTR_FLAG_USE_N ? jstr_likely(_n--) : 1)
		...
		_p += _rm.rm_so;
		if (jstr_unlikely(_ptnlen == 0)) {
			PJREG_DEB("jstr_unlikely(_ptnlen == 0)");
			++_p;
			if (jstr_unlikely(*_p == '\0'))
				break;
			continue;
		...
		if (*_cap > *_sz + _rplclen - _ptnlen) {
			PJREG_DEB("*_cap > *_sz + _rplclen - _ptnlen");
			if (_dst != _old) {
				PJREG_DEB("dst != old");
			...
			if (jstr_unlikely(_is_mmap)) {
				if (_dst != _old)
					memmove(_dst, _old, _p - _old);
				_tmp = *(uc **)_s;
				PJSTR_REALLOC(*_s, *_cap, *_sz + _rplclen - _ptnlen, return JREG_RET_MALLOC_ERROR);
				memmove(_p + _rplclen,
					_p + _ptnlen,
...
#endif /* HAVE_REALLOC_MREMAP */
			{
				...
				_tmp = (uc *)malloc(*_cap);
				PJSTR_MALLOC_ERR(_tmp, return JREG_RET_MALLOC_ERROR);
				if (_dst != _old) {
...
JSTR_FUNC
static jreg_errcode_ty
jreg_rplcall_len(char **JSTR_RST const _s,
		 size_t *JSTR_RST const _sz,
		 size_t *JSTR_RST const _cap,
		 const char *JSTR_RST const _rplc,
		 const size_t _rplclen,
		 ...
		 const int _eflags) JSTR_NOEXCEPT
{
	return pjreg_base_rplcall_len(PJSTR_FLAG_USE_NOT_N, _s, _sz, _cap, _rplc, 0, _rplclen, _preg, _eflags);
}
...
jreg_rplcall(char **JSTR_RST const _s,
		 ...
		 const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplcall_len(_s, _sz, _cap, _rplc, strlen(_rplc), _preg, _eflags);
}

JSTR_INLINE
JSTR_FUNC
static jreg_errcode_ty
jreg_rplcn_len(char **JSTR_RST const _s,
	       size_t *JSTR_RST const _sz,
	       size_t *JSTR_RST const _cap,
	       const char *JSTR_RST const _rplc,
	       const size_t _n,
	       ...
	       const int _eflags) JSTR_NOEXCEPT
{
	return pjreg_base_rplcall_len(PJSTR_FLAG_USE_N, _s, _sz, _cap, _rplc, _n, _rplclen, _preg, _eflags);
}
...
jreg_rplcn(char **JSTR_RST const _s,
	       ...
	       const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplcn_len(_s, _sz, _cap, _rplc, _n, strlen(_rplc), _preg, _eflags);
}

JSTR_INLINE
JSTR_FUNC
static jreg_errcode_ty
jreg_rplcall_now_len(char **JSTR_RST const _s,
		     size_t *JSTR_RST const _sz,
		     size_t *JSTR_RST const _cap,
		     const char *JSTR_RST const _ptn,
		     const char *JSTR_RST const _rplc,
		     ...
		     const int _eflags) JSTR_NOEXCEPT
{
	...
	if (jstr_unlikely(_ret != JREG_RET_NOERROR))
		return _ret;
	return jreg_rplcall_len(_s, _sz, _cap, _rplc, _rplclen, _preg, _eflags);
}
...
jreg_rplcall_now(char **JSTR_RST const _s,
		     ...
		     const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplcall_now_len(_s, _sz, _cap, _ptn, _rplc, strlen(_rplc), _preg, _cflags, _eflags);
}

JSTR_INLINE
JSTR_FUNC
static jreg_errcode_ty
jreg_rplcn_now_len(char **JSTR_RST const _s,
		   size_t *JSTR_RST const _sz,
		   size_t *JSTR_RST const _cap,
		   const char *JSTR_RST const _ptn,
		   const char *JSTR_RST const _rplc,
		   ...
		   const int _eflags) JSTR_NOEXCEPT
{
	...
	if (jstr_unlikely(_ret != JREG_RET_NOERROR))
		return _ret;
	return jreg_rplcn_len(_s, _sz, _cap, _rplc, _n, _rplclen, _preg, _eflags);
}
...
jreg_rplcn_now(char **JSTR_RST const _s,
		   ...
		   const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplcn_now_len(_s, _sz, _cap, _ptn, _rplc, _n, strlen(_rplc), _preg, _cflags, _eflags);
}

JSTR_INLINE
JSTR_FUNC
static jreg_errcode_ty
jreg_rplc_len(char **JSTR_RST const _s,
	      size_t *JSTR_RST const _sz,
	      size_t *JSTR_RST const _cap,
	      const char *JSTR_RST const _rplc,
	      const size_t _rplclen,
...
jreg_rplc(char **JSTR_RST const _s,
	      ...
	      const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplc_len(_s, _sz, _cap, _rplc, strlen(_rplc), _preg, _eflags);
}

JSTR_FUNC
JSTR_INLINE
static jreg_errcode_ty
jreg_rplc_now_len(char **JSTR_RST const _s,
		  size_t *JSTR_RST const _sz,
		  size_t *JSTR_RST const _cap,
		  const char *JSTR_RST const _ptn,
		  const char *JSTR_RST const _rplc,
...
jreg_rplc_now(char **JSTR_RST const _s,
		  ...
		  const int _eflags) JSTR_NOEXCEPT
{
	return jreg_rplc_now_len(_s, _sz, _cap, _ptn, _rplc, strlen(_rplc), _preg, _cflags, _eflags);
}
