...
JSTR_NOEXCEPT
{
	pmatch->rm_so = 0;
	pmatch->rm_eo = sz;
	return (jstr_re_errcode_ty)regexec(preg, s, nmatch, pmatch, eflags | JSTR_REG_EF_STARTEND);
}
#	define PJSTR_REG_EXEC(preg, s, sz, nmatch, pmatch, eflags) \
		jstr_re_exec_len(preg, s, sz, nmatch, pmatch, eflags)
#else
#	define PJSTR_REG_EXEC(preg, s, sz, nmatch, pmatch, eflags) \
		jstr_re_exec(preg, s, nmatch, pmatch, eflags)
...
JSTR_INLINE
static jstr_re_errcode_ty
jstr_re_match_now(const char *R s,
		  const char *R ptn,
		  regex_t *R preg,
		  const int cflags,
		  const int eflags)
...
{
	PJSTR_REG_COMP_NOW();
	return jstr_re_match(s, preg, eflags);
}
...
JSTR_INLINE
static jstr_re_errcode_ty
jstr_re_search_now(const char *R s,
		   const char *R ptn,
		   regex_t *R preg,
		   regmatch_t *R pmatch,
		   const int cflags,
...
{
	PJSTR_REG_COMP_NOW();
	return jstr_re_search(s, preg, pmatch, eflags);
}
...
JSTR_INLINE
static jstr_re_errcode_ty
jstr_re_search_len(const char *R s,
		   const size_t sz,
		   regex_t *R preg,
		   regmatch_t *R pmatch,
		   const int eflags)
...
JSTR_INLINE
static jstr_re_errcode_ty
jstr_re_search_len_now(const char *R s,
		       const char *R ptn,
		       const size_t sz,
		       regex_t *R preg,
		       regmatch_t *R pmatch,
...
{
	PJSTR_REG_COMP_NOW();
	return jstr_re_search_len(s, sz, preg, pmatch, eflags);
}
...
JSTR_INLINE
static jstr_re_errcode_ty
jstr_re_match_len(const char *R s,
		  const size_t sz,
		  regex_t *R preg,
		  const int eflags)
JSTR_NOEXCEPT
{
	regmatch_t rm;
	return jstr_re_exec_len(preg, s, sz, 0, &rm, eflags | JSTR_REG_EF_STARTEND);
}
...
JSTR_INLINE
static jstr_re_errcode_ty
jstr_re_match_len_now(const char *R s,
		      const char *R ptn,
		      const size_t sz,
		      regex_t *R preg,
		      const int cflags,
...
{
	PJSTR_REG_COMP_NOW();
	return jstr_re_match_len(s, sz, preg, eflags);
}
...
{
	regmatch_t rm;
	const jstr_re_errcode_ty ret = PJSTR_REG_EXEC(preg, s + start_idx, *sz - start_idx, 1, &rm, eflags);
	if (jstr_unlikely(ret != JSTR_REG_RET_NOERROR)
...
JSTR_NOEXCEPT
{
	return pjstr_re_rm(s, sz, 0, preg, eflags);
}

JSTR_FUNC
JSTR_INLINE
static jstr_re_errcode_ty
jstr_re_rm_now(char *R s,
	       size_t *R sz,
	       const char *R ptn,
	       regex_t *R preg,
	       const int cflags,
...
{
	PJSTR_REG_COMP_NOW();
	return jstr_re_rm(s, sz, preg, eflags);
}

JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rm_from(char *R s,
		size_t *R sz,
		const size_t start_idx,
		const regex_t *R preg,
		const int eflags)
JSTR_NOEXCEPT
{
	return pjstr_re_rm(s, sz, start_idx, preg, eflags);
}

JSTR_FUNC
JSTR_INLINE
static jstr_re_errcode_ty
jstr_re_rm_from_now(char *R s,
		    size_t *R sz,
		    const size_t start_idx,
		    const char *R ptn,
		    regex_t *R preg,
...
{
	PJSTR_REG_COMP_NOW();
	return jstr_re_rm_from(s, sz, start_idx, preg, eflags);
}

JSTR_FUNC
JSTR_INLINE
static jstr_re_errcode_ty
pjstr_re_rmall(char *R s,
	       size_t *R sz,
	       const regex_t *R preg,
	       const int eflags,
	       const int use_n,
...
JSTR_NOEXCEPT
{
	...
	size_t findlen;
	jstr_re_errcode_ty ret = JSTR_REG_RET_NOMATCH;
	while ((use_n & 1 ? n-- : 1)
...
JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rmall(char *R s,
	      size_t *R sz,
	      const regex_t *R preg,
	      const int eflags)
JSTR_NOEXCEPT
{
	return pjstr_re_rmall(s, sz, preg, eflags, 0, 0, 0);
}

JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rmall_now(char *R s,
		  size_t *R sz,
		  const char *R ptn,
		  regex_t *R preg,
		  const int cflags,
...
{
	PJSTR_REG_COMP_NOW();
	return jstr_re_rmall(s, sz, preg, eflags);
}

JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rmn(char *R s,
	    size_t *R sz,
	    const size_t n,
	    const regex_t *R preg,
	    const int eflags)
JSTR_NOEXCEPT
{
	return pjstr_re_rmall(s, sz, preg, eflags, 1, n, 0);
}

JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rmn_now(char *R s,
		size_t *R sz,
		const size_t start_idx,
		const char *R ptn,
		regex_t *R preg,
...
{
	PJSTR_REG_COMP_NOW();
	return jstr_re_rmn(s, sz, start_idx, preg, eflags);
}

JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rmall_from(char *R s,
		   size_t *R sz,
		   const size_t start_idx,
		   const regex_t *R preg,
		   const int eflags)
JSTR_NOEXCEPT
{
	return pjstr_re_rmall(s, sz, preg, eflags, 0, 0, start_idx);
}

JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rmall_from_now(char *R s,
		       size_t *R sz,
		       const size_t start_idx,
		       const char *R ptn,
		       regex_t *R preg,
...
{
	PJSTR_REG_COMP_NOW();
	return jstr_re_rmall_from(s, sz, start_idx, preg, eflags);
}
...
JSTR_FUNC
static jstr_re_errcode_ty
pjstr_re_rplcall_len(char *R *R s,
		     size_t *R sz,
		     size_t *R cap,
		     const char *R rplc,
		     const size_t rplclen,
...
JSTR_NOEXCEPT
{
	...
	while ((use_n & 1 ? n-- : 1)
		...
		if (rplclen <= findlen) {
			...
			pjstr_rplcallinplace(&dst, &oldp, (const u **)&p, (u *)rplc, rplclen, findlen);
		} else if (*cap > *sz + rplclen - findlen) {
			JSTR_PRINT_LOG("*cap > *sz + rplclen - findlen");
			pjstr_re_rplcall_small_rplc(*(u **)s, sz, &dst, &oldp, &p, (u *)rplc, rplclen, findlen);
		} else {
			JSTR_PRINT_LOG("else");
			if (jstr_unlikely(!pjstr_re_rplcall_big_rplc((u **)s, sz, cap, &dst, &oldp, &p, (u *)rplc, rplclen, findlen)))
				goto err;
		}
	}
	if (dst != oldp)
...
JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rplcall_len_from(char *R *R s,
			 size_t *R sz,
			 size_t *R cap,
			 const size_t start_idx,
			 const char *R rplc,
...
JSTR_NOEXCEPT
{
	return pjstr_re_rplcall_len(s, sz, cap, rplc, rplclen, preg, eflags, 0, 0, start_idx);
}

JSTR_FUNC
JSTR_INLINE
static jstr_re_errcode_ty
jstr_re_rplcall_len_from_now(char *R *R s,
			     size_t *R sz,
			     size_t *R cap,
			     const size_t start_idx,
			     const char *R ptn,
...
{
	PJSTR_REG_COMP_NOW();
	return jstr_re_rplcall_len_from(s, sz, cap, start_idx, rplc, rplclen, preg, eflags);
}

JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rplcn_len_from(char *R *R s,
		       size_t *R sz,
		       size_t *R cap,
		       const size_t start_idx,
		       const size_t n,
...
JSTR_NOEXCEPT
{
	return pjstr_re_rplcall_len(s, sz, cap, rplc, rplclen, preg, eflags, 1, n, start_idx);
}

JSTR_INLINE
JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rplcall_len(char *R *R s,
		    size_t *R sz,
		    size_t *R cap,
		    const char *R rplc,
		    const size_t rplclen,
...
JSTR_NOEXCEPT
{
	return pjstr_re_rplcall_len(s, sz, cap, rplc, rplclen, preg, eflags, 0, 0, 0);
}

JSTR_INLINE
JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rplcn_len(char *R *R s,
		  size_t *R sz,
		  size_t *R cap,
		  const char *R rplc,
		  const size_t n,
...
JSTR_NOEXCEPT
{
	return pjstr_re_rplcall_len(s, sz, cap, rplc, rplclen, preg, eflags, 1, n, 0);
}

JSTR_INLINE
JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rplcall_len_now(char *R *R s,
			size_t *R sz,
			size_t *R cap,
			const char *R ptn,
			const char *R rplc,
...
{
	PJSTR_REG_COMP_NOW();
	return jstr_re_rplcall_len(s, sz, cap, rplc, rplclen, preg, eflags);
}

JSTR_INLINE
JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rplcn_len_now(char *R *R s,
		      size_t *R sz,
		      size_t *R cap,
		      const char *R ptn,
		      const char *R rplc,
...
{
	PJSTR_REG_COMP_NOW();
	return jstr_re_rplcn_len(s, sz, cap, rplc, n, rplclen, preg, eflags);
}

JSTR_INLINE
JSTR_FUNC
static jstr_re_errcode_ty
pjstr_re_rplc_len(char *R *R s,
		  size_t *R sz,
		  size_t *R cap,
		  const char *R rplc,
		  const size_t rplclen,
...
JSTR_NOEXCEPT
{
	return pjstr_re_rplc_len(s, sz, cap, rplc, rplclen, preg, eflags, 0);
}

JSTR_INLINE
JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rplc_len_from(char *R *R s,
		      size_t *R sz,
		      size_t *R cap,
		      const size_t start_idx,
		      const char *R rplc,
...
JSTR_NOEXCEPT
{
	return pjstr_re_rplc_len(s, sz, cap, rplc, rplclen, preg, eflags, start_idx);
}

JSTR_INLINE
JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rplc_len_from_now(char *R *R s,
			  size_t *R sz,
			  size_t *R cap,
			  const size_t start_idx,
			  const char *R ptn,
...
{
	PJSTR_REG_COMP_NOW();
	return jstr_re_rplc_len_from(s, sz, cap, start_idx, rplc, rplclen, preg, eflags);
}

JSTR_FUNC
JSTR_INLINE
static jstr_re_errcode_ty
jstr_re_rplc_len_now(char *R *R s,
		     size_t *R sz,
		     size_t *R cap,
		     const char *R ptn,
		     const char *R rplc,
...
{
	PJSTR_REG_COMP_NOW();
	return jstr_re_rplc_len(s, sz, cap, rplc, rplclen, preg, eflags);
}
...
JSTR_INLINE
static jstr_re_errcode_ty
pjstr_re_rplc_bref_len(char *R *R s,
		       size_t *R sz,
		       size_t *R cap,
		       const char *R rplc,
		       size_t rplclen,
...
JSTR_NOEXCEPT
{
	if (jstr_unlikely(rplclen == 0))
		return jstr_re_rm_from(*s, sz, start_idx, preg, eflags);
	regmatch_t rm[10];
	jstr_re_errcode_ty ret = PJSTR_REG_EXEC(preg, *s + start_idx, *sz - start_idx, nmatch, rm, eflags);
	...
	const int has_bref = pjstr_re_strlen_rplc_dst(rm, (u *)rplc, rplclen, &rdstlen);
	...
	pjstr_re_creat_rplc_bref(*(u **)s, rm, (u *)rdst, (u *)rplc, rplclen);
...
JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rplc_bref_len(char *R *R s,
		      size_t *R sz,
		      size_t *R cap,
		      const char *R rplc,
		      size_t rplclen,
...
JSTR_NOEXCEPT
{
	return pjstr_re_rplc_bref_len(s, sz, cap, rplc, rplclen, preg, eflags, nmatch, 0);
}

JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rplc_bref_len_from(char *R *R s,
			   size_t *R sz,
			   size_t *R cap,
			   const size_t start_idx,
			   const char *R rplc,
...
JSTR_NOEXCEPT
{
	return pjstr_re_rplc_bref_len(s, sz, cap, rplc, rplclen, preg, eflags, nmatch, start_idx);
}

JSTR_INLINE
JSTR_FUNC
static jstr_re_errcode_ty
pjstr_re_rplcall_bref_len(char *R *R s,
			  size_t *R sz,
			  size_t *R cap,
			  const char *R rplc,
			  size_t rplclen,
...
JSTR_NOEXCEPT
{
	...
	if (jstr_unlikely(rplclen == 0))
		return pjstr_re_rmall(*s, sz, preg, eflags, use_n, n, start_idx);
	unsigned char *dst = p;
	...
	while ((use_n ? n-- : 1)
		...
		has_bref = pjstr_re_strlen_rplc_dst(rm, (u *)rplc, rplclen, &rdstlen);
		if (jstr_unlikely(has_bref == 0))
			return jstr_re_rplcall_len_from(s, sz, cap, start_idx, rplc, rplclen, preg, eflags);
		if (jstr_unlikely(rdstlen > JSTR_PAGE_SIZE)) {
		...
		pjstr_re_creat_rplc_bref((u *)p, rm, (u *)rdstp, (u *)rplc, rplclen);
		p += rm[0].rm_so;
		if (rdstlen <= findlen)
			pjstr_rplcallinplace(&dst, &oldp, (const u **)&p, rdstp, rdstlen, findlen);
		else if (*cap > *sz + rdstlen - findlen)
			pjstr_re_rplcall_small_rplc(*(u **)s, sz, &dst, &oldp, &p, rdstp, rdstlen, findlen);
		else if (jstr_unlikely(!pjstr_re_rplcall_big_rplc((u **)s, sz, cap, &dst, &oldp, &p, rdstp, rdstlen, findlen)))
			goto err_free;
	}
	if (dst != oldp)
		*sz = jstr_stpmove_len(dst, oldp, (*(u **)s + *sz) - oldp) - *s;
...
JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rplcall_bref_len(char *R *R s,
			 size_t *R sz,
			 size_t *R cap,
			 const char *R rplc,
			 size_t rplclen,
...
JSTR_NOEXCEPT
{
	return pjstr_re_rplcall_bref_len(s, sz, cap, rplc, rplclen, preg, eflags, nmatch, 0, 0, 0);
}

JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rplcn_bref_len(char *R *R s,
		       size_t *R sz,
		       size_t *R cap,
		       const char *R rplc,
		       size_t n,
...
JSTR_NOEXCEPT
{
	return pjstr_re_rplcall_bref_len(s, sz, cap, rplc, rplclen, preg, eflags, nmatch, 1, n, 0);
}

JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rplcall_bref_len_from(char *R *R s,
			      size_t *R sz,
			      size_t *R cap,
			      const size_t start_idx,
			      const char *R rplc,
...
JSTR_NOEXCEPT
{
	return pjstr_re_rplcall_bref_len(s, sz, cap, rplc, rplclen, preg, eflags, nmatch, 0, 0, start_idx);
}

JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rplcn_bref_len_from(char *R *R s,
			    size_t *R sz,
			    size_t *R cap,
			    const size_t start_idx,
			    const char *R rplc,
...
JSTR_NOEXCEPT
{
	return pjstr_re_rplcall_bref_len(s, sz, cap, rplc, rplclen, preg, eflags, nmatch, 1, n, start_idx);
}

JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rplcn_bref_len_from_now(char *R *R s,
				size_t *R sz,
				size_t *R cap,
				const size_t start_idx,
				const char *R ptn,
...
{
	PJSTR_REG_COMP_NOW();
	return pjstr_re_rplcall_bref_len(s, sz, cap, rplc, rplclen, preg, eflags, nmatch, 0, n, start_idx);
}

JSTR_INLINE
JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rplcn_bref_len_now(char *R *R s,
			   size_t *R sz,
			   size_t *R cap,
			   const char *R ptn,
			   const char *R rplc,
...
{
	PJSTR_REG_COMP_NOW();
	return jstr_re_rplcn_bref_len(s, sz, cap, rplc, n, rplclen, preg, eflags, nmatch);
}

JSTR_INLINE
JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rplcall_bref_len_now(char *R *R s,
			     size_t *R sz,
			     size_t *R cap,
			     const char *R ptn,
			     const char *R rplc,
...
{
	PJSTR_REG_COMP_NOW();
	return jstr_re_rplcall_bref_len(s, sz, cap, rplc, rplclen, preg, eflags, nmatch);
}

JSTR_INLINE
JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rplc_bref_len_now(char *R *R s,
			  size_t *R sz,
			  size_t *R cap,
			  const char *R ptn,
			  const char *R rplc,
...
{
	PJSTR_REG_COMP_NOW();
	return jstr_re_rplc_bref_len(s, sz, cap, rplc, rplclen, preg, eflags, nmatch);
}

JSTR_INLINE
JSTR_FUNC
static jstr_re_errcode_ty
jstr_re_rplc_bref_len_from_now(char *R *R s,
			       size_t *R sz,
			       size_t *R cap,
			       const size_t start_idx,
			       const char *R ptn,
...
{
	PJSTR_REG_COMP_NOW();
	return jstr_re_rplc_bref_len_from(s, sz, cap, start_idx, rplc, rplclen, preg, eflags, nmatch);
}
