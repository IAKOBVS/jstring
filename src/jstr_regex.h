...
#define PJSTR_CREAT_RPLC_BREF                                                                \
	do {                                                                                 \
		...
		PJSTR_CREAT_RPLC_BREF;
		if (jstr_unlikely(pjstr_rplcat_len(_s, _sz, _cap, _rm[0].rm_so, (char *)_rdst, _rdstlen, _ptnlen) == NULL))
			return JSTR_REG_RET_ENOMEM;
	}
	return _ret;
...
/* TODO: implement rplcall_len_bref */
			  ...
			  const size_t _nmatch) JSTR_NOEXCEPT
{
	...
	int _flag = HAS_NOT_BREF;
	size_t _ptnlen;
	for (size_t _off = 0; _off < *_sz; _off = _rm[0].rm_so + _rdstlen + 1) {
