...
pjstr_find_zero_low(pjstr_op_ty x)
{
	...
	pjstr_op_ty lsb = pjstr_repeat_bytes(0x01);
	pjstr_op_ty msb = pjstr_repeat_bytes(0x80);
	return (x - lsb) & ~x & msb;
}
...
pjstr_find_zero_all(pjstr_op_ty x)
{
	...
	pjstr_op_ty m = pjstr_repeat_bytes(0x7f);
	return ~(((x & m) + m) | x | m);
}

/* With similar caveats, identify bytes that are equal between X1 and X2.  */
static JSTR_INLINE pjstr_op_ty
pjstr_find_eq_low(pjstr_op_ty x1, pjstr_op_ty x2)
{
...
static JSTR_INLINE pjstr_op_ty
pjstr_find_zero_eq_low(pjstr_op_ty x1, pjstr_op_ty x2)
{
...
{
	pjstr_op_ty m = pjstr_repeat_bytes(0x7f);
	pjstr_op_ty eq = x1 ^ x2;
	pjstr_op_ty nz1 = ((x1 & m) + m) | x1;
	pjstr_op_ty ne2 = ((eq & m) + m) | eq;
	return (ne2 | ~nz1) & ~m;
}
