...
#define jarr_assign(j, ...)                                                     \
	do {                                                                    \
		JARRP_CHECK_ARG(j);                                             \
		JARRP_CHECK_VAL(j, JSTRP_PP_FIRST_ARG(__VA_ARGS__));            \
		if (jstr_unlikely(JARRP_CAP(j) < JSTRP_PP_NARG(__VA_ARGS__))) { \
			jarr_reserve(j, JSTRP_PP_NARG(__VA_ARGS__));            \
			JARRP_MALLOC_ERR(j, break)                              \
		}                                                               \
		JSTRP_PP_ARRCPY_VA_ARGS(JARRP_SZ(j), __VA_ARGS__);              \
		JARRP_SZ(j) = JSTRP_PP_NARG(__VA_ARGS__);                       \
	} while (0)
...
#define jarr_cat(j, ...)                                                                        \
	do {                                                                                    \
		JARRP_CHECK_ARG(j);                                                             \
		JARRP_CHECK_VAL(j, JSTRP_PP_FIRST_ARG(__VA_ARGS__));                            \
		if (jstr_unlikely(JARRP_CAP(j) < (JARRP_SZ(j) + JSTRP_PP_NARG(__VA_ARGS__)))) { \
			jarr_reserve(j, (JARRP_SZ(j) + JSTRP_PP_NARG(__VA_ARGS__)));            \
			JARRP_MALLOC_ERR(j, break)                                              \
		}                                                                               \
		JSTRP_PP_ARRCPY_VA_ARGS(JARRP_DATA(j) + JARRP_SZ(j), __VA_ARGS__);              \
		JARRP_SZ(j) += JSTRP_PP_NARG(__VA_ARGS__);                                      \
	} while (0)
/* Pop PTR[0]. */
