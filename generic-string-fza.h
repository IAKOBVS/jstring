...
pjstr_find_zero_low(jstr_op_ty x)
{
	...
	jstr_op_ty lsb = pjstr_repeat_bytes(0x01);
	jstr_op_ty msb = pjstr_repeat_bytes(0x80);
	return (x - lsb) & ~x & msb;
}

/* This function returns at least one bit set within every byte of X that
   is zero.  The result is exact in that, unlike pjstr_find_zero_low, all bytes
   are determinate.  This is usually used for pjstr_finding the index of the
   most significant byte that was zero.  */
static JSTR_INLINE jstr_op_ty
pjstr_find_zero_all(jstr_op_ty x)
{
	/* For each byte, pjstr_find not-zero by
	...
	jstr_op_ty m = pjstr_repeat_bytes(0x7f);
	return ~(((x & m) + m) | x | m);
}

/* With similar caveats, identify bytes that are equal between X1 and X2.  */
static JSTR_INLINE jstr_op_ty
pjstr_find_eq_low(jstr_op_ty x1, jstr_op_ty x2)
{
	return pjstr_find_zero_low(x1 ^ x2);
}
...
static JSTR_INLINE jstr_op_ty
pjstr_find_zero_eq_low(jstr_op_ty x1, jstr_op_ty x2)
{
	return pjstr_find_zero_low(x1) | pjstr_find_zero_low(x1 ^ x2);
}
...
pjstr_find_zero_ne_all(jstr_op_ty x1, jstr_op_ty x2)
{
	jstr_op_ty m = pjstr_repeat_bytes(0x7f);
	jstr_op_ty eq = x1 ^ x2;
	jstr_op_ty nz1 = ((x1 & m) + m) | x1;
	jstr_op_ty ne2 = ((eq & m) + m) | eq;
	return (ne2 | ~nz1) & ~m;
}

#endif /* PJSTR_GENERIC_STRING_FZA_H */
